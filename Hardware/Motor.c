...
void Motor3_SetPWM(int8_t PWM)
{
	if (PWM >= 0)							//如果设置正转的PWM
	{
		GPIO_ResetBits(GPIOA, GPIO_Pin_4);	//PB15置低电平
		GPIO_SetBits(GPIOA, GPIO_Pin_5);	//PB14置高电平
		PWM_SetCompare1(PWM);				//设置PWM占空比
	}
	else									//否则，即设置反转的速度值
	{
		GPIO_SetBits(GPIOA, GPIO_Pin_4);	//PB15置高电平
		GPIO_ResetBits(GPIOA, GPIO_Pin_5);	//PB14置低电平
		PWM_SetCompare1(-PWM);				//设置PWM占空比
	}
}

void Motor4_SetPWM(int8_t PWM)
{
	if (PWM >= 0)							//如果设置正转的PWM
	{
		GPIO_ResetBits(GPIOA, GPIO_Pin_1);	//PB15置低电平
		GPIO_SetBits(GPIOA, GPIO_Pin_0);	//PB14置高电平
		PWM_SetCompare2(PWM);				//设置PWM占空比
	}
	else									//否则，即设置反转的速度值
	{
		GPIO_SetBits(GPIOA, GPIO_Pin_1);	//PB15置高电平
		GPIO_ResetBits(GPIOA, GPIO_Pin_0);	//PB14置低电平
		PWM_SetCompare2(-PWM);				//设置PWM占空比
	}
}